    ...
    public static void main(String[] args) {
        ...
        post("/crearArticulo", (request, response) -> {
            ...
            if ((session.attribute("username") != null) && session.attribute("username").toString().trim() != "") {
                ...
                if (blogServices.crearArticulo(articulo) == true){
                     for (String etiq : etiquetas.split(",")) {
                         etiqueta.setIdetiqueta(idEtiqueta);
                         etiqueta.setIdarticulo(idArticulo);
                         etiqueta.setEtiqueta(etiq.toString().trim());
                         if (!etiqueta.getEtiqueta().toString().trim().isEmpty()){
                            blogServices.crearEtiqueta(etiqueta);
                         }
                         idEtiqueta +=1; 
                     }
                }
        ...
        get("/modificarArticulo", (request, response) -> {
            ...
            int cantEtiqueta =1;
            //Creando las etiquetas
            if (blogServices.modificarArticulo(art) == true){
                etiquetas = blogServices.listaEtiquetas(idarticulo);
                
                for (Etiqueta etiq : etiquetas) {
                    if (cantEtiqueta == 1){
                        etiqs = etiq.getEtiqueta().toString().trim();
                    }
                    else{
                        etiqs = etiqs + ", " + etiq.getEtiqueta().toString().trim();
                    }
                   cantEtiqueta +=1; 
                 }
            }
            atributos.put("etiquetas", etiqs);
            atributos.put("articulo", art);
        ...
        post("/modificarArticulo", (request, response) -> {
            ...
            blogServices.borrarEtiqueta(idarticulo);
